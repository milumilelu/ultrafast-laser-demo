...
| **C1** | 冻结范围 + 共用实验背景 | V2.0 | **接口完成** | `data/config/shared_experiment_background.json`：10 W 仅记录 / `E_p=P_物镜后/f` / w0·zR 由公式导出并**校验自洽**（差 0.033% / 0.007%）；焦点 `fixed_original_surface` + **保留** `axial_defocus`；**不改底层默认**（既有算例零影响）。⚠️ 三工作区界面收敛**未做**（本轮只做接口层） |
...
| **C6** | 端到端 + 精简回归 | V2.0 | **链路跑通** | `tools/e2e_core_v2.py` 五环节 6 秒跑完；报告 `docs/reports/e2e_core_v2.md`。🔴 **两个负面结论**（都真实）：① 标定**过拟合**（训练 10.657→7.996 μm 变好、留出 3.648→7.020 μm 变差）；② h/N **无可行解**（覆盖率仅 4.9%–72.0%）—— 根因是**光斑 0.874 μm（直径 1.75 μm）< 间距 2–10 μm，扫描线不重叠**。⚠️ 待你确认：共享背景的 0.874 μm 光斑是否真用于这批 CSV。⚠️ 三工作区界面收敛**未做** |
