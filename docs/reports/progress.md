...
> 更新：2026-09-11｜版本：`0.9.0-k1`（M0 放行版 `0.1.0-m0`；D `0.2.0-d`；E `0.3.0-e1`；F `0.4.0-f1`；G `0.5.0-g1`；H `0.6.0-h1`；I `0.7.0-i1`；J `0.8.0-j1`）
...
| K | 界面接管 | V0.2 | **通过（待人工验收）** | 本地 Web 界面（静态前端 + 真实求解 API）替代 Streamlit；契约测试 27/0 + Python 契约测试 23/0 |
| — | T20 | 各阶段 | 进行中 | 回归与文档随批次推进；A–K 的回归与记录已完成 |
