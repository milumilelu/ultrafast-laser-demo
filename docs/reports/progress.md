...
> 更新：2026-09-13｜版本：`0.9.1`（批次元数据 `build_batch = "L"`；M0 放行版 `0.1.0-m0`；D `0.2.0-d`；E `0.3.0-e1`；F `0.4.0-f1`；G `0.5.0-g1`；H `0.6.0-h1`；I `0.7.0-i1`；J `0.8.0-j1`）
>
> ⚠️ **版本号口径修正（2026-09-13）**：此前写的 `0.9.0-k1` **不是合法 PEP 440**，
> 实测会让 `pip wheel .` 直接失败（`project.version must be pep440`）。
> 现版本号为 `0.9.1`，批次改由 `pyproject.toml` 的 `[tool.ufdemo] build_batch` 独立承载。
>
> ⚠️ **批次边界（2026-09-13）**：M 批（真实数据接入）**只完成了前置修正 M①**
> （U15 修 M² 公式 / U16 修斜面快捷路径）。数据接入主体（U04–U14）**尚未开工**。
> 因此 `build_batch` **仍为 `"L"`**，待 M 批整体交付时再改，避免批次号虚报进度。
...
> 〔2026-09-13 现状更正：`origin` **已配置**为
> `https://github.com/milumilelu/ultrafast-laser-demo.git`（本地分支跟踪 `origin/main`，
> 尚未 fetch/push）。上句为 2026-09-11 告警**原文**，保留不改，仅在此注明现状。〕
...
| J | T18、T19 | V0.2 | **通过（待人工验收）** | 斜入射投影 + 动态角度 + 首次交点可见性 + 法向厚度转换；局部窗口使用全局高度场进行遮挡检测，曲面面积积分已修正；G07 回归与示例包复核通过 |
| K | 界面接管 | V0.2 | **通过（待人工验收）** | 本地 Web 界面（静态前端 + 真实求解 API）替代 Streamlit；契约测试 32/0 + Python 契约测试 35/0（2026-09-13 复测更新，原记 27/0 与 23/0） |
| L | 发布与接口加固 | V0.2 | **通过** | 统一 HTTP 错误序列化（F01）+ 合法版本号与 wheel 资源定位（F02/F03）+ 曲线单位映射与浏览器级回归（F04）+ 绑定提交的发布证据（F07）+ 读历史网格与快照元信息（F09）。pytest 418 passed；浏览器探针 29/0/1；wheel 冒烟 21/0 |
| M① | 复审前置修正 | V0.2 | **通过** | M²/瑞利长度校验公式方向修正（F10，独立反例 z_R 差 4 倍）+ 轴向光束＋初始斜面快捷路径补投影与法向换算（F11，端到端复现 `ln2·δ` 应为 `ln(√2)·√2·δ`，高估 41.42%）+ 浏览器探针两处挂死修复。物理输出与修复前**逐位一致**（16/16）。pytest 426 passed；验收 168/0/2；浏览器 30/0/0 |
| M②a | **U04** 实测数据导入 | V0.2 | **通过** | 导入 65+4 条（7 CSV，sha256 全一致）+ 可用性 QA 21→25 项 + 数据包版本核对表（定采用 9-11 版；与复审 77 条版的编号**整体位移一位**，引用须标用哪套）+ 曲线入口分层 `entry_class`（fixture 与实测分列；`ysz_analytic_depth_vs_fluence` 改名 `analytic_fixture_depth_vs_fluence`）。pytest 435 |
| M②b | **U05** 数据集权限门槛 | V0.2 | **通过** | 观测权限与增量权限**分开**：69 条实测全部 `observation_access=True`、`increment_access` **全 0**；硬门槛（语义不明/单位冲突/模拟量冒充/累计当增量）抛**既定错误码**不降级；软门槛（牌号未知、脉宽未确认）只进 `risk_notes` 不阻塞观测。`GET /api/datasets`。红线回归：三个能力入口测试 44 passed、材料卡 11 张 sha256 未变、曲线枚举仍 6 项。pytest 462 |
| M③ | **U06** 金刚石过程响应评估器 | V0.2 | **通过（未达验证门槛）** | 二次响应面 + 岭回归（α=0.01 固定）；**指标完全复现审计包**：训练 1.06/9.13/4.75%，分组五折 6.02/**52.42**/26.07%，留出 1.34/3.26/6.09%；留出行无关性系数最大差 0.00e+00。**未达自设门槛 → 不得标「已验证」**（代码判定）。界面新 tab + `/api/diamond-evaluator` + `/api/evaluator-predict`（不触发求解）。pytest 476；验收 174-0-2；浏览器 36-0-0 |
| M④ | **U07** 查表逐事件核 | V0.2 | **通过** | `TabulatedEventLaw`：仅 `event_depth_increment` 可进主循环（复用既有闸门，抛既定码）；插值复用 `tables.lookup`（逐点一致）；上界拒绝、下界默认拒绝，仅显式 `threshold_rule` 才记 0 且须带理由；`build_pulse_law` 按曲线分派。**`curve=None` 路径与改动前逐位一致（16/16）**。pytest 495；浏览器 36-0-0 |
| M⑤ | **U08** 文献算例回放 | V0.2 | **通过** | 4 组陶瓷/玻璃算例（点坑/交叉线），实测值+条件+Table 1|2 来源逐条可见；三条红线写成代码（等效脉冲数措辞、3D 重建标签、回放无增量权限）；源码级断言无「累计÷N」路径。界面新 tab + `/api/cases`。pytest 507；验收 180-0-2；浏览器 42-0-0 |
| M⑥ | **U09** CFRP 原始数据接入 | V0.2 | **通过** | 作者 XLSX 已**实际下载并解析**（sha256 `6d9747ae…`、157952 B、10 表、230 行）；manifest 状态 `located_not_downloaded` → `downloaded_and_parsed`。解析器按**表头文字**定位（第一版硬编码列号 → 只解出 8/10 表）。新增 `dataset_class`：观测包 69 **不并入**效率曲线包 230。pytest 518；验收 180-0-2 |
| M⑦ | **U10** AlSiC 数据补位 | V0.2 | **未完成（如实报缺）** | **未取得**可用飞秒数值（全文/图表在订阅墙后；本轮中英文各检索一次，证据已存 manifest）。**不是牌号不匹配**，是拿不到数值。已加守门测试：数据包**零纳秒混入、零相近材料冒充**；缺口带原因+排除项记录；材料卡 `delta_m` 保持 null。pytest 524 + 1 xfailed |
| **C1** | 冻结范围 + 共用实验背景 | V2.0 | **接口完成** | `data/config/shared_experiment_background.json`：10 W 仅记录 / `E_p=P_物镜后/f` / w0·zR 由公式导出并**校验自洽**（差 0.033% / 0.007%）；焦点 `fixed_original_surface` + **保留** `axial_defocus`；**不改底层默认**（既有算例零影响）。**界面已收敛**（C6）：8 tab → 三工作区 + 开发工具（原 6 面板折叠移入）；删 `mockSolve` 死代码；顶部 badge 改正；`app.py` 标 LEGACY；新增 8 条 V2 接口。pytest 594 / 浏览器探针 42-0 |
| **C2** | 上游轮廓对照 | V2.0 | **接口完成（虚拟输入）** | `src/ufdemo/upstream.py`：读 ``upstream_case.json``+``profile.csv``（兼容旧 Q4 列名）；**几何混用硬拦**（轴对称坑 ↔ 单线横截面）+ 半径定义换算提示（w=√2·w_old）+ 四类对照指标（中心深度 / 半高全宽 / 面积 / 轮廓 RMSE）。虚拟样例落盘 `examples/upstream/synthetic_pit_demo/`（**标 is_synthetic**）。⚠️ **等真实上游轮廓**才能做真对照 |
| **C4** | CSV 标定 | V2.0 | **机制完成 + 闭合验证** | GB18030 探测 + 中文表头映射 + 间距 mm→μm；负值保留、缺字段单列；工况组不跨训练/留出；`a` 进**几何更新之前** —— 闭合验证恢复 2.0000（误差 0.00%），有离焦时 D(2)/D(1)=1.0203（偏离线性 48.98%）。⚠️ **真实材料标定受阻**：现有卡基线与共享背景**不同光学**（ZrO₂ 卡为 16 μm 光斑协议 vs 共享背景 0.874 μm）→ 准入正确拦截，需先有同光学基线（C3） |
| **C3** | 反推基线 Fth/δ | V2.0 | **初值已出（闭合验证通过）** | AlSiC 223 fs：**Fth=7.850 J/cm²、δ=3652.5 nm**；训练中位相对误差 43.7% / 留出 58.7%（**初值级别，非可用模型**）。**闭合验证 0.00%** —— 证明机制对，误差来自模型/数据。**必须跨频率**才有解（单频率时 Fth 与 δ 共线，已硬拦）；可辨识性 δ×2→1.152、Fth×2→**0.239**（Fth 欠辨识）。报告 `docs/reports/c3_baseline_estimate.md`，候选值 `data/baselines/alsic_223fs_candidate.json` |
| **C5** | h/N 规划 | V2.0 | **完成（含用户修正）** | 弓字形 + 间距不拉伸 + 焦点 Z 恒定 + 时间标未校准 + 中立路径导出；`evaluate_candidate()` / `plan_for_target()`：25 候选**每个都真跑求解器**、**与标定共用同一条装配路径**（测试断言深度完全相等）、无解时给原因 + **明确标注的 best_effort**（含槽形貌）。**用户修正（2026-09-15）**：① 实际单线宽度 **5 μm** 作为声明的工艺事实 → 反推等效光斑（模型宽度=5.000 μm），覆盖率由 5–24% 改善到 37.5–100%；② 输出改「间距 h + **层数 N**」；③ 展示**矩形槽**形貌（热图 + 垂直扫描截面 + 起伏统计）。⚠️ 实测仍无可行解 —— 覆盖好的候选深度 ≥30 μm，见 C6 |
| **C6** | 端到端 + 精简回归 | V2.0 | **链路跑通** | `tools/e2e_core_v2.py` 五环节 6 秒跑完；报告 `docs/reports/e2e_core_v2.md`。🔴 **两个负面结论**（都真实）：① 标定**过拟合**（训练 10.657→7.996 μm 变好、留出 3.648→7.020 μm 变差）；② h/N **无可行解**（覆盖率仅 4.9%–72.0%）—— 根因是**光斑 0.874 μm（直径 1.75 μm）< 间距 2–10 μm，扫描线不重叠**。⚠️ 待你确认：共享背景的 0.874 μm 光斑是否真用于这批 CSV。**界面已收敛并接上 C2–C5**（见 C1 行）；pytest 594-1xf / 探针 42-0-1（U03 九路径全绿） |
| **L①** | 窗口半径策略（超阈值开窗） | V2.0 | **完成（默认关闭）** | 用户报「域 400 / 加工区 200 / dx 0.5 下 25 个候选 65 分钟」。**两次更正自己的错误结论**：① 上一轮「成本随网格超线性、有 per-event 全局开销」被对照实验推翻（`fixed_geometry` 下域 100/200/400 µm 单事件 131/132/138 µs，**网格涨 16 倍成本只涨 5%**）；② 本轮发现 `5fa5630` 的「局部迭代窗口」是**回归** —— 用**窗口内**起伏定尺寸时，光束脚下若是未加工平地而 20 µm 外是深坑，窗口会缩到 4 µm，把离焦 `u*=A/e` 处（ablating 半径**峰值** 19.95 µm）那段仍能烧的环带切掉（事件 40 实测：局部窗 8×9 只算出 20 个非零格，真实 87 个）。**已回退**（默认恢复全网格跨度，与 `e79892a` **逐位一致**，最大差 0.000e+00）。真因是 `axial_defocus` 自增强回路：深度 **132 µm** ≫ zR 4.47 ⇒ 光斑 1.33→**39 µm** ⇒ ε 窗口 **237 µm** 吃满域，而窗口内**超阈值**格子只占 **0.8%**。新策略用**与深度无关的严格上界** `r = w0·sqrt(A/(2e))·margin`（A = 未离焦峰值/阈值；`f(u)=u·ln(A/u)` 在 `u*=A/e` 取极大值 ⇒ 对任意 u 成立）。实测（均**逐位一致**）：域200/dx1 1.40→0.24 s（5.9×）、域400/dx0.5 21.91→1.34 s（16.3×）、**界面参数 域400/加工区200/dx0.5 77.58→3.21 s（24.2×）**，裁掉 92.9–95.4% 窗口格；按 279,600 事件换算 **~88 min → ~3.7 min**。**已接界面**（规划页新增「窗口半径策略」下拉 + 耗时预估**分口径** + `plan_for_target`/`plan_payload` 透传）。口径变化如实上报（`fluence_ledger.threshold_window` / 候选 `windowCellsSkippedFraction` / 候选 notes）；缺阈值·非法余量**报错**，solver 取不到阈值**回退+警告**，`grouped` 路径不支持并警告。ADR-0017。⚠️ 物理侧 δ 仍是 C3 的「初值级别」值（3652.5 nm）。pytest **622-1xf**；验收见下 |
| **L②** | 焦平面恒为原始上表面（用户明确） | V2.0 | **完成** | 用户 2026-09-15 明确「**焦点就固定在上表面**」。查证：该约定原先只是 `shared_experiment_background.json` 里的**文档** —— `focus_strategy` 全仓库只被回显两处、**无任何校验**；而 `geometry_feedback` 的**默认值是 `fixed_geometry`**（离焦恒 0、光斑随表面走，**等价于逐层重新对焦**）；`build_row_config` 调 `serpentine_plan` 时**没传 `focus_z_m`**，焦平面被**硬编码 0.0**。⇒ 做成三重约束：① 声明层 `solver.focus_strategy` 只收 `fixed_original_surface`，其它值 `CONFIG_INVALID`；② 构造层 `PredictionSpec.initial_height_m` 同时写进 `grid.initial_height_m` 与**每一段** z；③ 校验层 `validate_run` 复核「每段端点 z == `grid.initial_height_m`」，不等即**失败关闭**（挡住「声明对、路径却把焦点放别处」）。另记 `result.metadata['focus']`（策略 / z_m / layer_refocus=false / geometry_feedback / passive_defocus）⇒ 每次运行的光学假设可审计。ADR-0018；`tests/test_focus_convention.py` **12 条**（含行为断言：初始面 `axial_distance≈0` + 打深后 `max w > 1.5·w0` ⇒ 焦点固定且存在被动离焦）。既有 11 个示例路径 z 全为 0、`initial_height_m` 全为 0 ⇒ **不改变任何既有算例**。pytest **634-1xf** |
| **L③** | 路径连续性 / 加工区口径 / 细核重判（用户审查） | V2.0 | **完成（第 3、4 条）** | 外部审查（对 `e4c1106`）的 7 条逐条**在代码里复现**，本轮修掉已确认的 4 处：① **弓字形遍间断点** —— 每遍从 x0 重启，遍间回程却按「继续蛇形」写成 `x0 if n_lines%2==1 else x1`，**偶数条扫描线**时回到 x1 ⇒ 200×200/h=8/N=2（26 条线）段间跳 **200 µm**（奇数条线恰好正确，所以只在一半情形出现）；改为恒回 x0，并加**硬不变式**「上一段终点 == 下一段起点」（不等即 CONFIG_INVALID，报哪段/实际值）——实测 **72 个组合（3 区域 × 6 间距 × 4 层数）0 处不连续**。② **关光策略自相矛盾** —— 策略写 `not_assumed`、文案说「不依赖关光能力」，代码却把换向段标 `emitting=False`；改为**显式假设**（`shutter_off_during_turns=True` + `assumed_ideal_shutter_not_confirmed_against_device`）并删掉矛盾文案；**不擅自改成出光**（设备能力未知，改了会动所有算例）。③ **非正方形被静默压方** —— `float(region_um[0])` 丢第二维、统计又按第一维双向裁 ⇒ (120,60) 的指标实际算在 120×120 上；改为接受标量或 `(wx,wy)`（长度必须恰好 2）、路径与统计两维都用、结果带 `machiningRegionSizeUm`。④ **细核后不重判可行** —— 原实现只检查「细核有没有生成形貌」，于是「粗筛可行 → 细核不可行」照样推荐；改为按粗筛排序取前 `fine_top_k`（默认 3）个用最终精度复核、**复核后重新判可行并重排**，粗筛全否时也允许由否转可，理由里明说「细网格复核后不满足约束」。ADR-0019；`tests/test_path_and_region_integrity.py` **83 条**（含审查点名的 72 组合段端点逐段比对）。pytest **717-1xf**。⚠️ **未做**（需先定方向）：光学参数随拟合漂移（第 1 条）、标定用 20 µm 小方块 + 统计口径（第 2 条）、覆盖率≠达标面积（第 5 条）、上游对照工况未匹配（第 6 条）、增益未绑定与 train/holdout 分组泄漏（第 7 条） |
| **L④** | 光学冻结：单线宽度降级为对照量（用户：一定要改） | V2.0 | **完成** | 用户 2026-09-15 明确「**这个一定要改**」（外部审查第 1 条）。曾经 `shared_background_patch(threshold_J_m2=…)` 会按「声明的实测单线宽度 5 µm」**反推等效光斑**并替换 w0、连带改 zR ⇒ 光学随**频率与假定的 F_th** 漂移（实测 F_th=78496 时 2/20/40/200 kHz → w0 1.1334/1.3254/1.4085/**1.7026** µm，zR 3.26→**7.37** µm；名义 0.8743/1.9429）。机制：反推方程`r_abl = w·sqrt(ln(F0/F_th)/2)`、`F0 = 2E/(πw²)`、`E = P/f` **里带 E**；且模型本来就有地方让频率影响宽度（`F0` 带 E，名义光学下 2.94–4.29 µm），再反解回 w0 = **频率依赖计入两次**；且 5 µm 是**加工结果**（多脉冲扫描线宽），不是单脉冲足迹直径，这个等号本身没论证。⇒ 决定：① 光学**冻结**到名义值（w0=0.8743、zR=1.9429），与频率/能量/阈值无关，patch **显式**写出 `rayleigh_range_m`；② 单线宽度降级为**对照量**（basis 记 `declared_line_width_um` / `model_first_shot_width_um` / `declared_over_model_width`），差得多去查 F_th/δ 或搭接模型；③ `equivalent_spot_radius_m` 降级为**诊断工具**、不得再出现在装配路径；④ `geometryBasis` 与前端文案同步（「光斑半径（名义光学，冻结）」）。实测：20 kHz 模型首击宽 **3.486 µm** vs 声明 5.0 ⇒ **1.434×**（各 f 下 1.26–1.70×）——差距现在**看得见**。规划量级（25 候选、above_threshold、20.9 s）：平均深度 22.4–102.8 µm（中位 61.9）、覆盖率 0.316–1.000（中位 0.658）；早期「覆盖率 5–24% → 100%」是反推光斑换来的，**不是模型变准**，本决定撤掉。⚠️ 下一轮应**重新标定 F_th/δ**（光学冻结后剩下的宽度差距才可辨识）。ADR-0020；`tests/test_optics_frozen.py` **5 条**。pytest **722-1xf** |
| **L⑤** | 标定观测口径显式化（审查第 2 条） | V2.0 | **完成（机制）** | 查证：`calibrate_payload` 曾**硬编码** `window_um=20.0` ⇒ 标定实际仿真的是 **20×20 μm 的弓字形面扫描**、统计口径=全区域均值；而标定真正用的实验 CSV 在**仓库外**（`../数据/AlSiC|CFRP|SiC|ZrO2.csv`，列：脉宽/频率/间距/重复加工次数/速度/mean_depth/Sa/Sq/Sz/min/max），**没有任何一列声明**「加工区域多大」「平均深度是哪个窗口的均值」。⇒ 改为**显式声明、失败关闭**：`calibrationWindowUm / calibrationRegionUm / calibrationObservationKind(full_region_mean|center_window_mean) / calibrationCenterWindowUm` 从请求传入，`ObservationSpec`（早已支持中央窗口口径）真正接上；结果新增 `observationBasis`（尺寸/口径/**assumed** 标记/说明），未声明时如实标注「按 20 μm 假设，需要使用者确认」。⚠️ **待用户提供**：这批实验实际扫了多大区域、mean_depth 怎么统计的（整区域还是中央窗口）。pytest **722-1xf** |
| **L⑥** | 参考协议外置：三层参数归属（用户评审「光斑半径是设备参数吧」） | V2.0 | **完成** | 用户质疑成立。**设备层本来就在**（`data/config/shared_experiment_background.json`：NA 0.45 / M² 1.2 → 名义 w0 = 0.874 µm，ADR-0020 冻结），问题是**源文献的装置条件被内联进了材料卡**（`reference_protocol.required_laser` 的 λ/τ/f/w0）。**关键证据：卡里的 w0 从不进入物理计算** —— `solver.py` / `response.py` 里 `spot_radius`/`w0` **零命中**；`a = δ·ln(F/F_th)` 是局域能流定律，δ 与 F_th 本就 w0 无关，卡里的 w0 只作**条件门禁**。⇒ 三层分离：材料属性→`data/materials/`；源文献装置→**新增** `data/protocols/<id>.json`（标 `kind=source_experiment_beam`）；本机设备→`data/config/`。卡片只留 `{protocol_id, protocol_file}`；`validity_domain.peak_fluence_J_m2` 同为光束量，改名 `reference_peak_fluence_J_m2` 一并迁入。**加载器** `materials.resolve_reference_protocol()` 装配回 `MaterialSpec.reference_protocol`，**键集是分离前的超集** ⇒ `config.py`/`references.py`/`webcontract.py` 零改动（仅 webcontract 两处读峰值能流改源，输出字段名不变）。解析基址先取卡片 `parents[2]` 再回退 `resource_root()`（生成器写临时目录/wheel 换 prefix 都自洽）。**坏引用一律报错**：缺文件 / `protocol_id` 不一致 / 只有 id 没有 file ⇒ `CONFIG_INVALID` + 精确 `field_path`，绝不返回空协议。**顺带修掉生成器可复现性缺陷**：原先 `Path.write_text` 在 Windows 写 CRLF，而 `.gitattributes` 是 `* text=auto eol=lf` ⇒ **git 看不到 diff 但 sha256 全变**（哈希护栏会误报「材料卡被改写」）；新增 `_write_text_lf()`，修正后重跑 **4 张卡字节未变、仅 7 张（有协议者）变化**。**一个必须知道的后果**：文献装置 16 µm 与本机名义 0.874 µm 相差 **18 倍**，合起来过 `validate_run` 必被 `CONDITION_MISMATCH` 拒 —— 不是缺陷，说明「复现文献条件」与「本机名义光学预测」是两种运行。ADR-0021；`tests/test_reference_protocol_files.py` **13 条**（结构/解析/如实报错，且三处结构断言对**改动前**的卡均判违规 = 非恒真）。pytest **737-1xf**；生成器 7 项准入探针全 PASS；F01/F02 哈希不变；端到端复算 44.0616/63.3547/62.6316 µm、621 事件与迁出前一致。哈希护栏**有意刷新一次**（逐卡前后哈希见 `docs/reports/material_card_schema_change_adr0021.md`） |
| — | T20 | 各阶段 | 进行中 | 回归与文档随批次推进；A–M⑦ + C1–C6 的回归与记录已完成（C6 界面收敛除外） |
...
| 交付物 | 位置 | 核对方式 |
|---|---|---|
| 契约层（序列化 ，复用 ui_service 全部纯逻辑） |  | （23 项） |
| 零依赖 HTTP 服务（stdlib，无 FastAPI/Flask） |  | 起真实服务跑端到端 |
| 前端（迁入并按需求改造，来源目录未动） | （含 ） | （27 项） |
| 前端接线真实求解器（删除本地 mock 路径） | 、 | 契约测试「真实求解返回 completed」 |
| 前端补批次 H/I/J 面板 | 、 | DOM 契约 + 新控件接线检查 |
| 「七材料能力入口」页签（三类分开显示） | 同上 | 契约测试含  |
| 设计决定记录（含两个实测 bug） |  | 八项决定可追溯 |
...
- `python -m pytest -q`：**394 passed**（A–C 63 + D 18 + E 61 + 界面冒烟 18 + F 查表 91 + G 分相 21 + H 阈值/入口/水印 42 + I 分组 26 + J 斜入射 23 + K 契约 23 + 版本一致性 3 + 回归 5）。
...
- **webui 契约测试**：**通过 27｜失败 0**（DOM 契约 9 + 后端契约 18）。
