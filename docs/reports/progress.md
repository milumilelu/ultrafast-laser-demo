...
> 更新：2026-09-11｜版本：`0.6.0-h1`（M0 放行版 `0.1.0-m0`；D 为 `0.2.0-d`；E 为 `0.3.0-e1`；F 为 `0.4.0-f1`；G 为 `0.5.0-g1`）
...
| — | T20 | 各阶段 | 进行中 | 回归与文档随批次推进；A–H 的回归与记录已完成 |
...
**V0.1 查表（F）交付完成，待人工验收。批次 G（分相结构）、批次 H（受限阈值协议/七材料能力入口/水印）均已交付，待人工验收。**
...
| 验收接入 | `tools/run_acceptance.py`（H 段：G09-threshold/entries/watermark 共 14 行） | 通过 114｜失败 0｜未运行 4 |
...
### G09 / 受限阈值协议 + 七材料能力入口 + 水印（批次 H，本批实测）
...
- `python -m pytest -q`：**319 passed**（A–C 63 + D 18 + E 61 + 界面冒烟 18 + F 查表 91 + G 分相 21 + H 阈值/入口/水印 42 + 回归 5）。
- `docs/reports/acceptance_report.md`：**通过 114｜失败 0｜未运行 4**（含 H 段 14 行）。
- 界面操作检查：**通过 20｜失败 0｜未运行 0**。
- 端到端演示可用性：**通过 13｜失败 0｜未运行 0**（前后端对接全链路）。
- 查表拒收检查：**通过 21｜失败 0**。
- G06 分相检查：**通过 13｜失败 0**（结构实例 2 个）。
- G09 受限阈值协议：**通过 9｜失败 0**（`g09_threshold_protocol.csv`）。
- 七材料能力入口：**探针 27/27 成立（未核验 0）**（开放 15 + 红线 11 + 缺口 1）。
...
> 批次 H 的「受限阈值协议/七材料能力入口」未运行项已随批次 H 交付撤销（仅保留「查表接入逐事件核」口径）。
...
- [x] 本阶段适用 G 测试有实测记录，失败与未运行项目未标为通过。
      （`acceptance_report.md`：通过 114｜失败 0｜未运行 4，逐一列出原因；
...
批次 H 已交付：受限阈值协议（`thresholds.py`）、七材料能力入口（`MATERIAL_ENTRIES` +
`verify_entry_enforcements`）、水印同源（`build_watermark`）、界面阈值图层与能力入口页。
以下为后续批次的依赖与**本批遗留**：

1. **逐事件核查表接入（查表进主循环）**：本批仍**保留不开放**。只有
...
3. **性能基线（B01–B04）与批量加速（批次 I / T16、T17）**：目前只记录单次运行
   `elapsed_s`，`accelerators.py` 仍为占位。
4. **斜入射与动态角度（批次 J / T18、T19）**：`geometry.py` 为占位；
   「分相截断 × 动态角度」在配置层仍互斥，几何意义未定义前不得混用。
5. **实验回归用例**：YSZ 图 14、SiC 图 6 原图/原表数字化后建立；当前只做到公式核查。
