...
| **L①** | 窗口半径策略（超阈值开窗） | V2.0 | **完成（默认关闭）** | 用户报「域 400 / 加工区 200 / dx 0.5 下 25 个候选 65 分钟」，要求改成真正局部更新。**先更正自己的错误结论**：上一轮「成本随网格超线性、存在 per-event 全局开销」被对照实验推翻 —— `fixed_geometry`（离焦恒 0）下域 100/200/400 µm 的单事件成本 131/132/138 µs，**网格涨 16 倍成本只涨 5%**，无隐藏 O(网格) 路径；成本 ≈ 0.06 µs × 窗口格数。真因是 `axial_defocus` 自增强回路：深度 **132 µm** ≫ zR 4.47 ⇒ 离焦 30× ⇒ 光斑 1.33→**39 µm** ⇒ ε 窗口直径 **237 µm**（吃满 200 µm 域），而窗口内**超阈值**格子只占 **0.8%**。新策略按 `r = w·sqrt(ln(F_peak/F_th)/2)·margin`（margin 默认 1.25）开窗；阈值型核在 `F<=F_th` 处返回**恰好 0** ⇒ 去除量**逐位不变**（`np.array_equal` 断言）。实测：域200/dx1 1.34→0.23 s（5.8×）、域400/dx0.5 19.76→**0.58 s（33.9×）**、界面实际参数 57.5→**1.62 s（35×）**，均逐位一致；按 279,600 事件换算 65 min → **~1.9 min**。`fluence_ledger.threshold_window` 如实上报裁掉比例与口径变化（截断文案**分口径**，启用时不得说成「能流尾部超出计算域」）；缺阈值/非法余量**报错**、solver 取不到阈值**回退+警告**、`grouped` 路径不支持并写警告。ADR-0017 + `docs/reports/window_radius_policy.md`。**默认关闭**，既有算例零影响。⚠️ 未接界面（开关/耗时预估/planner 形参）见报告待办。pytest **619-1xf**（+11）；验收 181-0-2；界面 24-0；端到端 13-0；U03 浏览器 42-0-1；G06 13-0 |
