...
| **C5** | h/N 规划 | V2.0 | **完成** | 弓字形 + 间距不拉伸 + 焦点 Z 恒定 + 时间标未校准 + 中立路径导出；`evaluate_candidate()` / `plan_for_target()`：25 候选**每个都真跑求解器**、**与标定共用同一条装配路径**（测试断言深度完全相等）、无解时给原因（区分太浅/太深/横向约束）、质量信息分「实验证据 vs 模型预测」。⚠️ 实测无可行解（见 C6） |
