...
> 更新：2026-09-11｜版本：`0.5.0-g1`（M0 放行版 `0.1.0-m0`；D 为 `0.2.0-d`；E 为 `0.3.0-e1`；F 为 `0.4.0-f1`）
...
>
> ✅ **已于 2026-09-11 按 B 方案重建解决**：`main` 现指向 `66237a7`
> （末端树与旧 `d798a41` 逐位一致，均 `dfe031d5…`）；`git fsck --full` 无 broken link /
> 无 missing，`git bundle --all` 与全量克隆均成功，克隆内 pytest 277 通过。
> **以上告警原文原样保留**，处置细节见「历史重建记录（B 方案，2026-09-11 已执行）」。
