...
> 更新：2026-09-11｜版本：`0.7.0-i1`（M0 放行版 `0.1.0-m0`；D 为 `0.2.0-d`；E 为 `0.3.0-e1`；F 为 `0.4.0-f1`；G 为 `0.5.0-g1`；H 为 `0.6.0-h1`）
...
| — | T20 | 各阶段 | 进行中 | 回归与文档随批次推进；A–I 的回归与记录已完成 |
...
放行条件为「G07、G08 通过；有实测误差和性能报告」（细则第 2 节 M3）。当前进度：
...
| **G07 斜入射与表面几何** | 批次 J（T18）未实施 | ❌ 阻塞 M3 |

> 结论：**G08 与性能报告均已具备，M3 仍差 G07（批次 J）**；批次 J 完成后方可申请 M3 放行。

**V0.1 查表（F）交付完成，待人工验收。批次 G（分相结构）、H（受限阈值协议/七材料能力入口/水印）、
I（冻结几何批量 + 性能基准）均已交付，待人工验收。**
...
### G08 / 分组模式与逐脉冲对照 + B01–B04（批次 I，本批实测）
...
- `python -m pytest -q`：**345 passed**（A–C 63 + D 18 + E 61 + 界面冒烟 18 + F 查表 91 + G 分相 21 + H 阈值/入口/水印 42 + I 分组 26 + 回归 5）。
- `docs/reports/acceptance_report.md`：**通过 126｜失败 0｜未运行 2**（含 I 段 12 行）。
- 界面操作检查：**通过 22｜失败 0｜未运行 0**（含批量模式切换与参考模式面板）。
...
- [x] 本阶段适用 G 测试有实测记录，失败与未运行项目未标为通过。
      （`acceptance_report.md`：通过 126｜失败 0｜未运行 2，逐一列出原因；
...
批次 I 已交付：冻结几何分组批量（`accelerators.py`）、局部误差估计与回退、可选 Numba 局部核、
B01–B04 性能基准、界面求解模式开关与批量面板。以下为后续批次的依赖与**本批遗留**：
...
3. **斜入射与动态角度（批次 J / T18、T19）**：`geometry.py` 为占位；
   「分相截断 × 动态角度」在配置层仍互斥，几何意义未定义前不得混用。
   **批次 J 是 M3 的唯一阻塞项**（G07）。
4. **「批量 + 动态角度」组合仍不开放**（细则 9.1 末）：只有组合测试通过后才可开放；
   当前在配置层拦截。
5. **分组批量可支持的工况扩展**：分批相（需表达跨相界面截断）、分历史耦合
   （需保持事件顺序依赖）、可支持更多几何反馈模式——均属后续工程扩展。
