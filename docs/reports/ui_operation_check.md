...
- 生成时间（UTC）：2026-09-10T14:45:36.796588+00:00
- 代码版本：commit=4748cb258c1e965431eb089e5116f8a3855291ea｜工作区有改动=True
...
| 检查 | 预期 | 实测 | 状态 | 说明 |
|---|---|---|---|---|
| 初始渲染无异常 | 无异常 | 无异常 | 通过 | 五标签页：参数与运行 / 结果 / 参考评估器 / 查表 / 历史运行 |
| 初始 solve_count | 0 | 0 | 通过 | 未提交前不得求解 |
| 初始无冻结结果 | frozen=None | frozen=None | 通过 |  |
| 提交计算：solve_count 恰好 +1 | 1 | 1 | 通过 | run_id=ui_run_20260910T144533_y0ht｜status=completed |
| 改参数未提交：旧结果标为「上一次运行」且不求解 | is_stale=True｜solve_count=1｜界面出现「上一次运行」 | is_stale=True｜solve_count=1｜label=上一次运行（2026-09-10T14:45:33.981515+00:00） | 通过 | 执行细则 11.3 |
| 改参数后 solve_count 不变 | 1 | 1 | 通过 |  |
| 快照回放 / 切图层 / 旋转视图 / 切截面：solve_count 不变 | 1（不变） | 1 | 通过 | 执行细则 11.3「只旋转视图、切换图层/截面/快照时读取已有数据」 |
| 读取历史运行：read_count +1 且 solve_count 不变 | read_count=1｜solve_count=0 | read_count=1｜solve_count=0｜run_id=ui_run_20260910T144533_y0ht | 通过 | 读取不等于用当前参数求解 |
...
| 合成模式禁止导出物理深度 | 以 depth_um 命名时拒绝；depth 标签非 um | 导出标签=depth/delta_ref｜拒绝 depth_um=True | 通过 | 执行细则 4.1：合成深度与微米平面坐标不可混算 |
...
| 探针运行输出位置 | 隔离到 C:\Users\RZF\Desktop\博士课题资料\工艺仿真软件\ultrafast-demo\runs\acceptance\_ui_probe | 已写入 22 个运行目录 | 通过 | UFDEMO_RUNS_DIR 覆盖，不污染工作区 runs/ |
