...
**文献只提供材料参数，光束用本机的即可 —— 这是成立的**：`a = δ·ln(F/F_th)` 里 `δ` 是绝对长度，
局域能流场在等比缩放下是自身的拷贝，所以**深度与光斑半径无关**。实测（同一张卡，保持
峰值能流 `F0`、有效脉冲数 `N_eff`、搭接比 `hatch/w₀` 不变，只扫 `w₀`）：

| w₀ (µm) | 16 | 8 | 4 | 2 | **0.874** |
|---|---:|---:|---:|---:|---:|
| 中心深度 (µm) | 62.660403 | 62.660403 | 62.660403 | 62.660403 | **62.660403** |

**光斑相差 18 倍，深度逐位相同（相对极差 4.5e-16）** ⟹ `w₀` 不是物理量，是尺度/机器描述。

**真正决定深度的是**：峰值能流 `F0`、**有效脉冲数 `N_eff`（累积制程）**、以及搭接比 `hatch/w₀`。
`λ`、`τ`、`N_eff` 现行门禁已正确管住；但门禁**额外**要求 `spot_radius_m` 与
`repetition_rate_Hz` 逐字匹配，会把「文献材料参数 + 本机光束」这条正确用法拦死
（`CONDITION_MISMATCH`）—— 这是**门禁口径问题**，不是物理限制。
详见 `docs/decisions/ADR-0021-reference-protocol-separation.md` 文末补记。

⚠️ **本机真正要解决的障碍是「搭接」**：本机名义 `w₀ = 0.874 µm`（直径 1.75 µm），
而工艺间距常在 2–10 µm 量级 → `hatch/w₀ ≫ 1`，扫描线不搭接。
实测（间距固定 4 µm）：事件 621 → **47**、中心深度 62.66 → **0 µm**（只剩孤立沟槽）。
