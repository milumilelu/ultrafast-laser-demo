"""⚠️ **LEGACY（V2 起退出主流程）** —— 旧 Streamlit 界面。

本文件**不再是本版本的启动入口**，也不再是必装依赖。
当前唯一入口是本地 Web 应用::

    PYTHONPATH=src python -m ufdemo.webapp --port 8787

保留它的原因：里面有些交互与诊断面板在排查时仍可参考。
按任务书 §7.2，等主流程稳定后再归档 —— **不同时维护两套 UI**。
